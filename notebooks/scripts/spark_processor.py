...
if DeltaTable.isDeltaTable(spark, "s3a://landing-zone/silver/dim_songs/"):
    ...
    deltaSongs.alias("target").merge(
     ...
     .execute()
    print("Songs Dimension updated via Merge.")
else:
    df_songs_dim_new.write.format("delta").save("s3a://landing-zone/silver/dim_songs/")
    print("Songs Dimension created (First Load).")
...
if DeltaTable.isDeltaTable(spark, "s3a://landing-zone/silver/dim_users/"):
    ...
    deltaUsers.alias("target").merge(
     ...
     .whenNotMatchedInsertAll() \
     .execute()
    print("Users Dimension updated via Merge.")
else:
    df_users_dim_new.write.format("delta").save("s3a://landing-zone/silver/dim_users/")
    print("Users Dimension created (First Load).")
