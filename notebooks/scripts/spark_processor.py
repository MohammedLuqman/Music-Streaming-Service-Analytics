# Generated from: spark_processor.ipynb
# Converted at: 2026-07-08T09:58:57.390Z
...
df_songs_dim_new = df_songs_bronze.select(
...
if DeltaTable.isDeltaTable(spark, "s3a://landing-zone/silver/dim_songs/"):
    deltaSongs = DeltaTable.forPath(spark, "s3a://landing-zone/silver/dim_songs/")
    deltaSongs.alias("target").merge(
        df_songs_dim_new.alias("source"),
        "target.song_id = source.song_id"
    ).whenMatchedUpdateAll() \
     .whenNotMatchedInsertAll() \
     .execute()
else:
    df_songs_dim_new.write.format("delta").save("s3a://landing-zone/silver/dim_songs/")
print("Songs Dimension processed via CDC/Merge.")

df_users_bronze = spark.read.option("header", "true").csv("s3a://landing-zone/users/")
df_users_dim_new = df_users_bronze.select(
...
if DeltaTable.isDeltaTable(spark, "s3a://landing-zone/silver/dim_users/"):
    deltaUsers = DeltaTable.forPath(spark, "s3a://landing-zone/silver/dim_users/")
    deltaUsers.alias("target").merge(
        df_users_dim_new.alias("source"),
        "target.user_id = source.user_id"
    ).whenMatchedUpdateAll() \
     .whenNotMatchedInsertAll() \
     .execute()
else:
    df_users_dim_new.write.format("delta").save("s3a://landing-zone/silver/dim_users/")
print("Users Dimension processed via CDC/Merge.")
